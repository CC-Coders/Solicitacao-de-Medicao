...
function buscaItensDoContrato(CODCOLIGADA, IDCNT) {
    try {
        var query = "";
        query += "SELECT ITEM.NSEQITMCNT, PRODUTO.IDPRD, PRODUTO.CODIGOPRD, PRODUTO.NOMEFANTASIA, MAX(MEDICOES.NSEQMEDICAO) as NSEQMEDICAO, COUNT(MEDICOES.NSEQMEDICAO) as QUANTIDADE, ISNULL(SUM(MEDICOES.VALOR),0) as VALORMEDICOES "
