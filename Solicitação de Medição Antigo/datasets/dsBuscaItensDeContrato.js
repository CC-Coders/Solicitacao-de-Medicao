...
function buscaItensDoContrato(CODCOLIGADA, IDCNT) {
    try {
        var query = "";
        query += "SELECT ITEM.NSEQITMCNT, PRODUTO.IDPRD, PRODUTO.CODIGOPRD, PRODUTO.NOMEFANTASIA, MAX(MEDICOES.NSEQMEDICAO) as NSEQMEDICAO, ITEM.QUANTIDADE, SUM(MEDICOES.VALOR) as VALORMEDICOES "
        ...
        query += "    LEFT JOIN TITMCNTMEDICAO MEDICOES ON CONTRATO.CODCOLIGADA = MEDICOES.CODCOLIGADA AND CONTRATO.IDCNT = MEDICOES.IDCNT AND ITEM.NSEQITMCNT = MEDICOES.NSEQITMCNT "
        query += "WHERE CONTRATO.CODCOLIGADA = ? AND CONTRATO.IDCNT = ? ";
        query += "GROUP BY ITEM.NSEQITMCNT,  PRODUTO.IDPRD, PRODUTO.CODIGOPRD, PRODUTO.NOMEFANTASIA, ITEM.QUANTIDADE";
